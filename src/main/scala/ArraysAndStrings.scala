...
object ArraysAndStrings {
  ...
  def merge(nums1: Array[Int], m: Int, nums2: Array[Int], n: Int): Unit = {
    ...
    while (writePointer >= 0) {
      val v1 = nums1.lift(readPointer1).getOrElse(Int.MinValue)
      val v2 = nums2.lift(readPointer2).getOrElse(Int.MinValue)
      nums1(writePointer) = v1 max v2
      if (v1 > v2) readPointer1 -= 1 else readPointer2 -= 1
      writePointer -= 1
    }
  }
